...
- `claimQualityReviewTask()`: only transitions `QUEUED -> CLAIMED`. Before claiming, it checks `reviewerRestriction.findFirst` for a restriction that is active right now, mirroring `lib/curriculum/review/eligibility.ts`'s predicate: `effectiveFrom <= now`, `liftedAt: null`, and (`effectiveUntil: null` or `effectiveUntil > now`), scoped to the task's school or global. This correctly catches a restriction with a future `effectiveUntil` (still active) and correctly ignores one that has been lifted or has not started yet. A restricted reviewer is rejected with `REVIEWER_RESTRICTED` (403). The actual state transition uses an optimistic-concurrency `updateMany` guarded by `{ id, version, status: "QUEUED" }`; a `count !== 1` result throws `TASK_VERSION_CONFLICT` (409). Writes a `quality_review.task.claimed` audit record.
...
**Authorization and eligibility fail closed.** `createQualityReviewTask()` reuses `assertReviewOperationsAdmin()` from the curriculum-review authority layer, so a school administrator can create only school-scoped work while platform and MOE-super administrators can create broader work. Claims and decisions require the active reviewer profile to belong to the acting user, match the task authority and school scope, remain available and unrestricted, and have a current verified credential at the required authority. A caller cannot claim or decide work for another reviewer profile.
...
`openCalibrationSession()` performs the explicit `DRAFT -> OPEN` transition with optimistic status guarding and a required audit record. It is limited to the existing review-operations administrator authority; `recordCalibrationResult()` continues to reject every non-`OPEN` session.
...
- **Applying the two canonical migrations to any real (staging or production) database.** `prisma/canonical/migrations/20260901_000001_add_quality_review_tasks/` and `prisma/canonical/migrations/20260901_000002_add_quality_review_calibration/` are part of this repository's canonical migration ledger (see `docs/adr/0017-layered-schema-authority.md`) and are exercised end-to-end, including Row Level Security, by `scripts/verify-full-canonical-bootstrap.ps1` against a disposable PostgreSQL 17 container. Both new tables from each migration have `ENABLE ROW LEVEL SECURITY` applied from creation, matching this repository's server-only table convention; no anon/authenticated grants are made. What remains external is applying them to any real staging or production database, which requires the same separate database approval this repository requires for every canonical migration. In particular, `QualityReviewTask.claimedByProfileId`'s `ON DELETE SET NULL` foreign-key clause was verified against a real PostgreSQL 17 instance by the canonical bootstrap verifier, but has not been exercised against live staging or production data.
