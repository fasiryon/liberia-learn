...
**Known limitation: no code path transitions DRAFT to OPEN.** `QualityReviewCalibrationSession.status` defaults to `DRAFT` in the schema, and `createCalibrationSession()` never sets a different status. `recordCalibrationResult()` requires `status === "OPEN"`. Nothing in this repository currently moves a session from `DRAFT` to `OPEN`; this mirrors an identical, pre-existing gap in the curriculum-review calibration module, not a new problem introduced here. A real deployment needs an explicit session-open action (an operator or scheduled step) before `recordCalibrationResult()` can ever succeed against a freshly created session.
