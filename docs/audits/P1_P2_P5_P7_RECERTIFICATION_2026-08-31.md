...
| P7 | P7-A | Versioned governed taxonomy for dosage, retention, mastery, adoption, workflow, helpfulness, grounding, hallucination and safety; privacy cohorts, synthetic exclusion and complete metric definitions | `docs/EVENT_TAXONOMY.md` and `lib/metrics`, `lib/analytics`, and AI-quality modules provide partial telemetry. Synthetic-load exclusion exists for selected dashboards. No current source defines the complete required taxonomy, governed cohort policy, or all required metric metadata | No P7 operational evidence exists | PARTIAL | Implement a versioned P7 metric registry and contract tests before NR-15. |
...
**WAIT FOR REPOSITORY-LOCAL P7 GAPS.** P7-A, P7-B, and P7-C are not complete.
