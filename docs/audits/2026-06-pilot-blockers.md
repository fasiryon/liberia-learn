...
| **A3** — Observability env | **RESOLVED (for outreach gate)** | Set in Vercel Prod: `SENTRY_DSN` + `NEXT_PUBLIC_SENTRY_DSN` (project `liberialearn-web`), `OPS_ALERT_EMAIL`, `OPS_ALERT_PHONE`, `SQS_DLQ_URL`, `SQS_QUEUE_URL`, `RESEND_API_KEY`. **Sentry drill PASSED** (`student1` → malformed JSON `POST /api/discussion/posts` → HTTP 500 → Sentry issue **LIBERIALEARN-WEB-T**, `SyntaxError … is not valid JSON`, ~2 min). **A3 RESOLVED for outreach gate.** Sentry verified live and capturing events in production (drill confirmed LIBERIALEARN-WEB-T). Email alert delivery deferred to Sprint 5B — `RESEND_API_KEY` is set but no verified sending domain. Alerting is a pre-pilot operational concern, not a pre-outreach concern. (Tracked as **B20** in `docs/audits/2026-06-pre-pilot-blockers.md`.) |
...
- **No verified email sender in prod (A3 fallout).** `RESEND_API_KEY` is now set, but **no Resend sending domain is verified** — a live API test was rejected (`liberialearn.edu.lr` not verified; default `FROM` `noreply@liberialearn.edu.lr`). So every ops alert *and all transactional email* (`sendEmail` → guardian digests, etc.) is **undeliverable**. SMS inert too (`sms:"unavailable"`, no AT/Twilio creds). High priority for pilot. Fix = verify a domain at resend.com/domains + set `EMAIL_FROM` to that domain + redeploy.
