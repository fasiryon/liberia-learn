...
Definitions are versioned and validate lifecycle, explicit basis-point allocation, a control arm, governed P7-A metric IDs and current metric version, eligibility/exclusion rules, conflict domains, and predeclared early-stop policies. Mandatory child-safety controls cannot vary by arm. Definitions that attempt it are rejected. A material change to arms, allocation, metrics, eligibility, conflict domains, or stop policy requires a new definition version; historical semantic hashes remain stable across lifecycle changes.

Assignment is derived deterministically with FNV-1a from experiment/version, assignment unit, tenant, and unit ID. It is repeatable across requests and restarts. Minimum client versions use strict numeric SemVer comparison. Assignment records carry the assigned school, and an exposure for another school is rejected. Assignment is not exposure. Exposure is logged only on encounter through P7-A's `governed.experiment.exposure` contract, with a stable operation identity including session and feature context. Offline clients preserve event time, version, arm, assignment identity, and synthetic-source marker for replay-safe ingestion.
