...
| NR-7 | Systematic Tenant Access Guard | 2 | Security | COMPLETE (not yet merged) | PASS (2026-08-01, branch `feat/nr-7-tenant-guard`, commit `37f9c5f9`, not pushed). Reused the pre-existing `lib/tenant/assert-school-access.ts` guard (found already built but only applied to 3 routes from a pre-plan May commit). Triaged all 109 HIGH/MEDIUM-flagged routes across `app/api/{student,teacher,admin}/` (out of 367 in scope); found and fixed 3 genuine cross-tenant gaps (LessonVideo admin cross-school management + listing leak, post-change-eval GET missing tenant check entirely, agents/* platform-wide-data routes reachable by any school ADMIN). Extended the tenant-isolation test pattern with 20 new tests covering ~26 distinct routes. Gate: prisma generate/tsc/vitest 4466 tests-542 files (baseline 4446/541)/build all PASS, zero schema changes. See `CURRENT_EXECUTION_STATE.md` for full detail and the scope caveat (the ~258 non-flagged routes were not individually re-read). |
| NR-8 | RBAC Expansion + SSO Onboarding Fix | 2 | Security | COMPLETE (not yet merged) | PASS (2026-08-01, branch `feat/nr-8-rbac-sso`, not pushed). A May 22 pre-plan commit had already built most of deliverables 1-2 (assertPermission on 12 governance/export/override routes, Google SSO invite gate). This sprint added MOE_SUPER_ADMIN/MOE_DISTRICT_ADMIN to SessionUser's role type (deliverable 3), found and fixed 11 MOE routes that excluded MOE_SUPER_ADMIN via a literal "MOE_OFFICIAL" string check (2 of which used a dead "PLATFORM_ADMIN" role string that could never match a real user), and hardened Google SSO to block an existing TEACHER row with a null schoolId, not just block new-user creation. Gate: prisma generate/tsc/vitest 4488 tests-543 files (baseline 4466/542)/build all PASS, zero schema changes. See `CURRENT_EXECUTION_STATE.md` for full detail. |
...
| NR-15 | Unified Ops Dashboard + Alerting | 4 | Ops | COMPLETE AND CERTIFIED | PR #124 merged to `main` at `54f3a33232cd05861007c1f40011b5a8c03de7a9` |
