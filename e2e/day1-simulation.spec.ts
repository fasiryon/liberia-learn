...
test("Gate 3  Student: morning lesson experience", async ({ page }) => {
  ...
  await expect(
    page
      .getByText(
        /today's schedule|today's subjects|current lesson|day summary|smart continue|nothing scheduled|timetable not configured/i
      )
      .first()
...
test("Gate 8  MOE: national dashboard", async ({ page }) => {
  await page.goto(`${baseUrl}/moe/login`);
  await page.waitForTimeout(MEDIUM);
...
test("Gate 9  Timetable: student schedule view", async ({ page }) => {
  ...
  const hasEmptyState = todayBody.toLowerCase().match(/no lesson|nothing scheduled|empty/i);
