...
export function student_browse() {
  ...
  const todayRes = http.get(`${BASE_URL}/api/student/today`, { headers: HEADERS });
  check(todayRes, { "today 200": (r) => r.status === 200 });
  sleep(0.5);

  // 2. Lesson catalogue page 1
  const lessonsRes = http.get(`${BASE_URL}/api/student/lessons?page=1`, { headers: HEADERS });
  ...
  const leagueRes = http.get(`${BASE_URL}/api/league`, { headers: HEADERS });
