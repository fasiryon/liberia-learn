 ...
 */
function generateHomeworkQuestions(subject: string, label: string, hwIndex: number): string[] {
  const base = [
...
async function seedSchool(
  ...
  for (let ti = 0; ti < CLASS_SUBJECTS.length; ti++) {
    ...
    for (let ai = 0; ai < 5; ai++) {
      ...
      const hwQuestions = generateHomeworkQuestions(cs.subject, cs.label, ai);
