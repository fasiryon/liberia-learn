...
export function GuidedOnboarding({ reopenKey = 0, onClose }: GuidedOnboardingProps) {
  ...
  return (
    ...
    >
      <div className="relative mx-4 w-full max-w-md rounded-2xl border border-emerald-500/40 bg-slate-900 p-7 shadow-2xl">
        {/* Progress bar */}
        ...
        {/* Content */}
        <h2 className="mb-3 text-xl font-bold text-slate-50">{current.title}</h2>
        <p className="mb-7 text-sm leading-relaxed text-slate-300">{current.body}</p>

        {/* Action */}
        <button
          onClick={handleNext}
          className="w-full rounded-xl bg-emerald-500 px-5 py-3.5 text-sm font-semibold text-slate-950 hover:bg-emerald-400 active:scale-[0.98] transition-all"
        >
