...
export function ConsentAcceptanceModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center bg-[var(--ll-bg)]/90 px-4 py-6 backdrop-blur"
      role="dialog"
      aria-modal="true"
      aria-labelledby="policy-consent-title"
    >
      <div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)] p-5 shadow-none shadow-black/60 sm:p-6">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--ll-yellow)]">
          Required policy acceptance
        </p>
        <h2 id="policy-consent-title" className="mt-3 text-2xl font-semibold text-[var(--ll-text)]">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <Link
            href="/legal/privacy"
            target="_blank"
            className="min-h-11 rounded-xl border border-[var(--ll-border)] px-4 py-3 text-center text-sm font-semibold text-[var(--ll-text)] hover:border-emerald-400"
          >
            View Privacy Policy
          </Link>
          <Link
        ...
        <div className="mt-4 rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/70 p-4 text-xs leading-5 text-[var(--ll-text-muted)]">
          Policy version: <span className="font-semibold text-[var(--ll-text)]">{policyVersion}</span>. Acceptance is recorded
        ...
        <button
          ...
          className="mt-5 flex min-h-12 w-full items-center justify-center rounded-xl bg-[var(--ll-yellow-soft)] px-4 py-3 text-sm font-bold text-[var(--ll-text-faint)] hover:bg-[var(--ll-yellow-soft)] disabled:opacity-60"
        >
