  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button type="button" aria-label="Close protractor" className="rounded border border-[var(--ll-border)] px-2 py-1 text-xs" onClick={() => onClose?.()}>Close</button>
      </div>

      <div className="flex items-center gap-2 text-xs">
        <button type="button" aria-label="Toggle full circle mode" className="rounded border border-[var(--ll-border)] px-2 py-1" onClick={() => setFullCircle((v) => !v)}>
          {fullCircle ? "360 deg" : "180 deg"}
        </button>
        <label>
          Angle
      ...
      >
        ...
        <text x="130" y="20" textAnchor="middle" fill="#cbd5e1" fontSize="12">{angle} deg</text>
      </svg>
    </div>
  );
}
