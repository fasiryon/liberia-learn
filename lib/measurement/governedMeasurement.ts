...
export const EVENT_REGISTRY: readonly EventDefinition[] = [
  ...
  definition({ name: "governed.safety.decision", family: "safety_decisions", actorTypes: ["system", "reviewer"], requiredMetadata: ["decision", "classifierVersion"], optionalMetadata: ["reviewOutcome"], sourceSubsystem: "safety moderation", privacy: "restricted", timestampSemantics: "server_decision" }),
...
export const METRIC_REGISTRY: readonly MetricDefinition[] = [
  ...
  metric({ id: "teacher_adoption", displayName: "Meaningful teacher adoption", owner: "Teaching Experience", sourceEvents: ["governed.teacher.meaningful_action"], numerator: "teachers with a meaningful teaching action", denominator: "eligible teachers observed in scope", eligibility: "teacher with valid tenant scope", window: "rolling 28 days", missingData: "unobserved teachers are not inferred active", grain: "school", unit: "proportion", directionality: "higher_is_better", caveats: "account creation and login are excluded" }),
  ...
  metric({ id: "safety_decisions", displayName: "Safety decision coverage", owner: "Child Safety", sourceEvents: ["governed.safety.decision", "governed.safety.review"], numerator: "valid allowed, blocked, escalated, unavailable, or malformed decisions", denominator: "AI interactions requiring moderation decisions", eligibility: "decision and classifier version recorded", window: "rolling 28 days", missingData: "classifier unavailable is a named decision, never allowed", grain: "school, classifier version", unit: "proportion", directionality: "higher_is_better", caveats: "review outcomes explicitly distinguish false positives and false negatives; telemetry contains categories, not safeguarding content" }),
] as const;
...
export function ingestGovernedEvents(inputs: unknown[]): IngestionResult {
  ...
  for (const input of inputs) {
    ...
    const identity = result.event.operationId || result.event.sourceEventId || result.event.eventId;
...
export type MetricResult = { metricId: MeasurementFamily; metricVersion: number; numerator: number; denominator: number; value: number | null; includedEventSchemaVersions: number[]; excludedSyntheticEvents: number; missingDataEvents: number; schoolId: string; window: { start: string; end: string } };
export function calculateMetric(metricId: MeasurementFamily, events: GovernedEvent[], schoolId: string, window: { start: string; end: string }): MetricResult {
  const start = Date.parse(window.start), end = Date.parse(window.end);
  const inWindow = events.filter((e) => e.schoolId === schoolId && e.syntheticSource === "production" && Date.parse(e.occurredAt) >= start && Date.parse(e.occurredAt) <= end);
  const excludedSyntheticEvents = events.filter((e) => e.schoolId === schoolId && e.syntheticSource !== "production").length;
  ...
  if (metricId === "retention") { const entries = named("governed.retention.cohort_entered").filter((e) => e.actorId); const returns = new Set(named("governed.retention.learning_day").map((e) => e.actorId)); denominator = entries.length; numerator = entries.filter((e) => returns.has(e.actorId)).length; }
  ...
  if (metricId === "teacher_adoption") { const rows = named("governed.teacher.meaningful_action").filter((e) => typeof e.metadata.action === "string"); numerator = denominator = new Set(rows.map((e) => e.actorId).filter(Boolean)).size; }
  if (metricId === "workflow_completion") { const starts = named("governed.workflow.started").filter((e) => typeof e.metadata.workflowId === "string"); const done = new Set(named("governed.workflow.completed").map((e) => String(e.metadata.workflowId))); denominator = starts.length; numerator = starts.filter((e) => done.has(String(e.metadata.workflowId))).length; }
  ...
  if (metricId === "safety_decisions") { const rows = named("governed.safety.decision").filter((e) => typeof e.metadata.classifierVersion === "string"); denominator = rows.length; numerator = rows.filter((e) => ["allowed", "blocked", "escalated", "classifier_unavailable", "classifier_malformed"].includes(String(e.metadata.decision))).length; }
  return { metricId, metricVersion: GOVERNED_METRIC_VERSION, numerator, denominator, value: denominator ? numerator / denominator : null, includedEventSchemaVersions: [GOVERNED_EVENT_SCHEMA_VERSION], excludedSyntheticEvents, missingDataEvents, schoolId, window };
}
