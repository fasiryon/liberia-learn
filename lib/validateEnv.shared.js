const GROUPS = {
  ...
  auth: {
    ...
    requiredWhen: [
      {
        when: ["PRIVILEGED_MFA_ENFORCEMENT_ENABLED"],
        required: [
          "AUTH0_CLIENT_ID",
          "AUTH0_CLIENT_SECRET",
          "AUTH0_ISSUER",
    ...
    optional: ["PRIVILEGED_STEP_UP_MAX_AGE_SECONDS"],
  },
  aiProviders: {
    label: "AI providers",
    requiredWhen: [
      ...
      {
        when: [
          "ENABLE_CANVA_CERTIFICATES",
          "ENABLE_CANVA_MOE_REPORTS",
          "ENABLE_CANVA_COURSE_THUMBNAILS",
          "ENABLE_SCHOOL_ONBOARDING_KITS",
          "ENABLE_CERTIFICATION_ASSET_GENERATION",
        ],
        required: ["ANTHROPIC_API_KEY"],
      },
      {
        when: ["ENABLE_HIGGSFIELD_VIDEO_GENERATION"],
        required: ["HIGGSFIELD_CREDENTIALS"],
      },
    ],
    optional: [
      "ANTHROPIC_API_KEY",
      "ANTHROPIC_CANVA_MODEL",
      "GROQ_API_KEY",
  ...
  mediaStorage: {
    ...
    requiredWhen: [
      {
        when: ["ENABLE_LESSON_AUDIO_GENERATION"],
        required: ["SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY", "SUPABASE_LESSON_AUDIO_BUCKET"],
      },
    ],
    recommended: ["SUPABASE_LESSON_AUDIO_BUCKET"],
  },
  workerQueue: {
    label: "worker queue",
    requiredWhen: [
      {
        when: ["ENABLE_CURRICULUM_REGEN_QUEUE"],
        required: ["SQS_QUEUE_URL", "AWS_REGION"],
      },
    ],
    optional: ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_SESSION_TOKEN", "AWS_PROFILE"],
  },
  optionalDemoDev: {
    label: "optional demo/dev",
    optional: ["DEMO_MODE", "DEMO_RESET_SECRET", "DEMO_SCHOOL_IDS", "ALLOW_LIVE_DEMO_RESET", "LOG_LEVEL"],
  },
};
