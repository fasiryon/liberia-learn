    ...
    <main className="min-h-screen bg-[var(--ll-bg)] text-[var(--ll-text)]">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="mb-1 text-xs uppercase tracking-wide text-[var(--ll-yellow)]">Governance</p>
            ...
            {PERIODS.map((option) => (
              <Link
                ...
                className={`rounded-full border px-3 py-1.5 text-xs ${
                  option === period
                    ? "border-emerald-500 bg-[var(--ll-yellow)]/20 text-[var(--ll-yellow)]"
                    : "border-[var(--ll-border)] text-[var(--ll-text)] hover:bg-[var(--ll-bg)]"
                }`}
              >
                {option}
              </Link>
        ...
        <section className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-4">
            <p className="text-xs text-[var(--ll-text-muted)]">Audit Events</p>
            <p className="mt-2 text-2xl font-bold text-[var(--ll-yellow)]">
              {report.overview.auditEvents.toLocaleString()}
            </p>
          </div>
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-4">
            <p className="text-xs text-[var(--ll-text-muted)]">Exports</p>
            <p className="mt-2 text-2xl font-bold text-[var(--ll-silver)]">
              {report.overview.exportsGenerated.toLocaleString()}
            </p>
          </div>
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-4">
            <p className="text-xs text-[var(--ll-text-muted)]">AI Actions</p>
            <p className="mt-2 text-2xl font-bold text-violet-300">
              {report.overview.aiActions.toLocaleString()}
            </p>
          </div>
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-4">
            <p className="text-xs text-[var(--ll-text-muted)]">Sensitive Actions</p>
            <p className="mt-2 text-2xl font-bold text-[var(--ll-yellow)]">
              {report.overview.sensitiveActions.toLocaleString()}
            </p>
          </div>
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-4">
            <p className="text-xs text-[var(--ll-text-muted)]">Active Admins</p>
            <p className="mt-2 text-2xl font-bold text-[var(--ll-text)]">
              {report.overview.activeAdmins.toLocaleString()}
            </p>
          </div>
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-4">
            <p className="text-xs text-[var(--ll-text-muted)]">Affected Schools</p>
        ...
        <section className="mb-6 grid gap-4 xl:grid-cols-[1.1fr,0.9fr]">
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <Link href="/admin/compliance" className="text-xs text-[var(--ll-yellow)] hover:underline">
                Full audit log
              </Link>
            </div>
            {report.adminActions.byAction.length === 0 ? (
              <p className="text-sm text-[var(--ll-text-muted)]">No admin actions recorded in this period.</p>
            ) : (
              <div className="space-y-2">
                {report.adminActions.byAction.slice(0, 10).map((action) => (
                  ...
                  >
                    <div>
                      <p className="text-sm font-semibold text-[var(--ll-text)]">{action.action}</p>
                      <p className="text-xs text-[var(--ll-text-faint)]">
                        {action.sensitive ? "Sensitive" : "Standard"} governance action
                      </p>
                    </div>
                    <p className="text-lg font-bold text-[var(--ll-text)]">{action.count}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-5">
            ...
            {report.exportActivity.byType.length === 0 ? (
              <p className="text-sm text-[var(--ll-text-muted)]">No exports generated in this period.</p>
            ) : (
              <div className="space-y-2">
                {report.exportActivity.byType.map((entry) => (
                  <div
                    ...
                    className="flex items-center justify-between rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/60 px-4 py-3"
                  >
                    <p className="text-sm font-semibold text-[var(--ll-text)]">{entry.exportType}</p>
                    <p className="text-lg font-bold text-[var(--ll-silver)]">{entry.count}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        <section className="mb-6 grid gap-4 xl:grid-cols-[1fr,1fr]">
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-5">
            ...
            {report.aiActions.byFeature.length === 0 ? (
              <p className="text-sm text-[var(--ll-text-muted)]">No AI actions recorded in this period.</p>
            ) : (
              <div className="space-y-2">
                {report.aiActions.byFeature.slice(0, 8).map((entry) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-sm font-semibold text-[var(--ll-text)]">{entry.feature}</p>
          ...
          <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-5">
            ...
            {report.sensitiveActionLog.length === 0 ? (
              <p className="text-sm text-[var(--ll-text-muted)]">No sensitive actions recorded in this period.</p>
            ) : (
              <div className="space-y-2">
                {report.sensitiveActionLog.slice(0, 8).map((entry) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-[var(--ll-text)]">{entry.action}</p>
                        <p className="text-xs text-[var(--ll-text-faint)]">
