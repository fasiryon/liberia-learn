    ...
    <main className="min-h-screen bg-slate-950 text-slate-50">
      <div className="mx-auto max-w-6xl px-4 py-6 space-y-4">
        <header className="flex items-center justify-between rounded-2xl border border-slate-800 bg-slate-900/80 px-4 py-3">
          <div>
            <p className="text-[10px] uppercase text-emerald-300">LiberiaLearn - Admin</p>
        ...
        {user.isPlatformAdmin ? (
          <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 text-sm">
            ...
            >
              ...
              <div className="space-y-1">
                <label className="text-[11px] text-slate-300">Primary color (hex)</label>
                <input
                  name="primaryHex"
                  defaultValue="#22c55e"
                  className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50 outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400/60"
              ...
              <div className="space-y-1">
                <label className="text-[11px] text-slate-300">Secondary color (hex)</label>
                <input
                  name="secondaryHex"
                  defaultValue="#0ea5e9"
                  className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50 outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400/60"
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 text-sm">
          ...
          {schools.length === 0 ? (
            <p className="text-xs text-slate-400">No schools available.</p>
          ) : (
            <div className="space-y-2">
              {schools.map((school) => (
                <Link
                  ...
                  className="flex items-center justify-between rounded-xl bg-slate-950/80 px-3 py-3 transition-colors hover:bg-slate-900"
                >
                  <div>
                    <p className="text-slate-100">{school.name}</p>
                    <p className="text-[11px] text-slate-400">{school.timezone}</p>
                  </div>
