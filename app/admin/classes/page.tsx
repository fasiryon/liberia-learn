...
export default function AdminClassesPage() {
  ...
  return (
    <main className="min-h-screen bg-[#05060a] text-[var(--ll-text)] p-6">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-2xl font-semibold">Admin � Classes</h1>
        <p className="text-[var(--ll-text)]/60 mt-1">Create at least one class so Homework can be tenant-scoped.</p>

        <div className="mt-6 rounded-xl border border-[var(--ll-border)] bg-white/5 p-5 space-y-3">
          <label className="grid gap-1">
            <span className="text-sm text-[var(--ll-text)]/80">Class name</span>
            <input
              value={name}
          ...
          <label className="grid gap-1">
            <span className="text-sm text-[var(--ll-text)]/80">Subject</span>
            <select
              value={subject}
        ...
        <div className="mt-6 rounded-xl border border-[var(--ll-border)] bg-white/5 p-5">
          <div className="text-[var(--ll-text)]/80 font-semibold mb-2">Existing classes</div>
          {rows.length === 0 ? (
            <div className="text-[var(--ll-text)]/60 text-sm">No classes yet.</div>
          ) : (
            <div className="space-y-2">
              {rows.map((r) => (
                <div key={r.id} className="rounded-xl border border-[var(--ll-border)] bg-[#0b0d14] p-3 text-sm">
                  <div className="text-[var(--ll-text)]/90">
                    {r.name} <span className="text-[var(--ll-text)]/50">({r.subject})</span>
                  </div>
                  <div className="text-[var(--ll-text)]/60 text-xs">id: {r.id}</div>
                </div>
              ))}
            </div>
          )}
