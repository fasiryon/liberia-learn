    ...
    <main className="min-h-screen bg-[var(--ll-bg)] px-4 py-8 text-[var(--ll-text)]">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--ll-text-muted)]">Your official school-year record, ready for promotion and graduation workflows.</p>
        </div>

        {loading ? <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/70 p-6 text-sm text-[var(--ll-text-muted)]">Loading transcript...</div> : null}
        ...
        <div className="grid gap-4">
          {transcripts.map((transcript) => {
            ...
            return (
              <section key={transcript.id} className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/70 p-6">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    <p className="text-xs uppercase tracking-[0.2em] text-[var(--ll-yellow)]">Transcript Record</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/60 px-4 py-3">
                      <p className="text-xs uppercase tracking-wide text-[var(--ll-text-faint)]">Grade</p>
                      <p className="mt-1 text-lg font-semibold text-[var(--ll-text)]">Grade {transcript.grade}</p>
                    </div>
                    <div className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/60 px-4 py-3">
                      <p className="text-xs uppercase tracking-wide text-[var(--ll-text-faint)]">GPA</p>
                      <p className="mt-1 text-lg font-semibold text-[var(--ll-text)]">
                ...
                <div className="mt-5">
                  <h3 className="text-sm font-semibold text-[var(--ll-text)]">Summary</h3>
                  {examAuthority ? (
                    <div className="mt-3 rounded-xl border border-emerald-400/20 bg-[var(--ll-yellow)]/10 p-4">
                      <div className="grid gap-3 md:grid-cols-4">
                        <div>
                          <p className="text-xs uppercase tracking-wide text-[var(--ll-yellow)]">Published Exams</p>
                          <p className="mt-1 text-lg font-semibold text-[var(--ll-text)]">{examAuthority.publishedExamCount ?? 0}</p>
                        </div>
                        <div>
                          <p className="text-xs uppercase tracking-wide text-[var(--ll-yellow)]">Passed Exams</p>
                          <p className="mt-1 text-lg font-semibold text-[var(--ll-text)]">{examAuthority.passedExamCount ?? 0}</p>
                        </div>
                        <div>
                          <p className="text-xs uppercase tracking-wide text-[var(--ll-yellow)]">Average Score</p>
                          <p className="mt-1 text-lg font-semibold text-[var(--ll-text)]">
                            {examAuthority.averageScorePct != null ? `${examAuthority.averageScorePct}%` : "Pending"}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs uppercase tracking-wide text-[var(--ll-yellow)]">Last Release</p>
                      ...
                      {(examAuthority.results?.length ?? 0) > 0 ? (
                        <div className="mt-4 grid gap-3 md:grid-cols-2">
                          {examAuthority.results?.map((result) => (
                            <div key={result.examId} className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/50 p-4">
                              <p className="text-sm font-semibold text-[var(--ll-text)]">{result.title}</p>
                  ...
                  {entries.length === 0 ? (
                    <p className="mt-2 text-sm text-[var(--ll-text-muted)]">No summary has been published for this year yet.</p>
                  ) : (
                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                      {entries.map(([key, value]) => (
                        <div key={key} className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/60 p-4">
                          <p className="text-xs uppercase tracking-wide text-[var(--ll-text-faint)]">{key.replace(/_/g, " ")}</p>
                          <p className="mt-2 text-sm text-[var(--ll-text)]">
