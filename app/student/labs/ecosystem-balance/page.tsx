...
export default async function StudentEcosystemBalanceLabPage() {
      ...
      <main className="ll-page min-h-screen px-4 py-8 text-[var(--ll-text)]">
        <div className="ll-shell max-w-5xl">
          <section className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/80 p-6 text-[var(--ll-text)]">
            Ecosystem Balance Lab is not available yet.
          </section>
        </div>
      </main>
    );
  ...
  await logLearningEvent({
    ...
    target: { type: "ai_lab", id: "ecosystem-balance" },
    eventType: "LAB_OPENED",
    source: "/student/labs/ecosystem-balance",
    metadata: {
    ...
    <main className="ll-page min-h-screen px-4 py-8 text-[var(--ll-text)]">
      <div className="ll-shell max-w-6xl">
        <div className="mb-5">
          ...
          <h1 className="mt-2 text-3xl font-semibold text-[var(--ll-text)]">Ecosystem Balance Lab</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--ll-text)]">
            Explore predator-prey relationships and how populations interact in an ecosystem.
          </p>
        </div>
        <EcosystemBalanceLabPage />
      </div>
    </main>
  );
}
