...
import { approveAction } from "@/lib/autonomous/actions/approvalDecisionService";
...
export async function POST(req: NextRequest, { params }: { params: { approvalRequestId: string } }) {
  try {
    ...
    return NextResponse.json({ error: error?.message ?? "Failed to approve action" }, { status: error?.status ?? 500 });
  }
}
