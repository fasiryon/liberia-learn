...
export async function GET(req: NextRequest) {
  try {
    ...
    await auditPredictiveRead({ user, route: "/api/admin/ops/trajectories", scope });
    const result = await getPredictiveIntelligence({ scope, range });
    return NextResponse.json({
      enabled: result.enabled,
      trajectories: result.forecasts.map((forecast) => ({
        forecastId: forecast.id,
        type: forecast.type,
        trajectory: forecast.trajectory,
        riskBand: forecast.riskBand,
        confidenceScore: forecast.confidenceScore,
        freshness: forecast.forecastFreshness,
        factors: forecast.contributingFactors.map((factor) => ({ key: factor.key, direction: factor.direction, score: factor.score })),
      })),
