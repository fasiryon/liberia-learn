    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-xl bg-violet-500 px-4 py-2 text-sm font-semibold text-[var(--ll-text)] hover:bg-violet-400"
        >
            ...
            {[
              ...
              <div key={field.key}>
                ...
                <input
                  ...
                  className="w-full rounded-lg bg-[var(--ll-bg)] border border-[var(--ll-border)] px-3 py-2 text-sm text-[var(--ll-text)] focus:outline-none focus:border-violet-500"
                />
              </div>
            ))}
          </div>
      ...
      <section className="rounded-xl border border-[var(--ll-border)] bg-[var(--ll-bg)]/70 p-6">
        {loading ? (
          ...
          <p className="text-sm text-[var(--ll-text-muted)]">No schools registered.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {schools.map((s) => (
                  ...
                  >
                    <td className="py-3 pr-3 font-medium text-[var(--ll-text)]">
                      {s.name}
                      {s.motto && (
                        <span className="block text-[11px] text-[var(--ll-text-faint)] mt-0.5">
                          {s.motto}
                        </span>
                      )}
                    </td>
                    ...
                    <td className="py-3 pr-3 text-xs">
                      {s.contactName ?? "--"}
                      {s.contactEmail && (
                        <span className="block text-[var(--ll-text-faint)]">{s.contactEmail}</span>
                      )}
                    </td>
                      ...
                      <div className="flex gap-1">
                        {s.status !== "ACTIVE" && (
                          <button
                            ...
                            className="rounded-lg bg-[var(--ll-yellow)]/20 px-2 py-1 text-[11px] text-[var(--ll-yellow)] hover:bg-[var(--ll-yellow)]/30"
                          >
                            Activate
                          </button>
                        )}
